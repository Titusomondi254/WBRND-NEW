...
<html lang="en">
  <head>
    <title>Commutes and Destinations Map - Walbrand Movers</title>
    <meta charset="utf-8"/>
    ...
    <style>
      ...
      .commutes-map .map-view .error-message {
        position: absolute;
        inset: 0;
        display: flex;
        align-items: center;
      ...
      .commutes-map .map-view .error-message strong {
        display: block;
        margin-bottom: 8px;
        color: #d93025;
        font-size: 1.05rem;
  ...
  <body>
    ...
    <script>
      const CONFIGURATION = {
        ...
        "mapOptions": {"center":{"lat":-0.023559,"lng":37.906193},"fullscreenControl":true,"mapTypeControl":false,"streetViewControl":false,"zoom":14,"zoomControl":true,"maxZoom":20,"mapId":""},
        "mapsApiKey": "AIzaSyChidKg7Jxuzfx87haL5THWmdKfzwp11PI"
      };
      ...
      function showCommutesWidgetError(message) {
        const mapView = document.querySelector('.commutes-map .map-view');
        if (!mapView) return;
        mapView.innerHTML = `
          <div class="error-message">
            <strong>Google Maps cannot load.</strong>
            ${message}
          </div>`;
      }

      function handleMapsScriptError() {
        console.error('Google Maps script failed to load.');
        showCommutesWidgetError('Please verify the API key, billing, and referrer restrictions in Google Cloud Console.');
      }

      window.gm_authFailure = function() {
        console.error('Google Maps authentication failure.');
        showCommutesWidgetError('Authentication failed. Check API key restrictions, billing, and enabled Maps APIs.');
      };
    </script>
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyChidKg7Jxuzfx87haL5THWmdKfzwp11PI&callback=initMap&libraries=places,geometry&v=weekly&solution_channel=GMP_QB_commutes_v3_c" async defer onerror="handleMapsScriptError()"></script>
  </body>
</html>